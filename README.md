...
<p align="center"><strong>别再只看 Star。用可复核证据判断 GitHub 项目是否可信、是否值得采用。</strong></p>

<p align="center">
  它不是一个“假 Star 检测器”，也不会凭异常比例给项目定罪。它把可信度、置信度、采用风险和证据缺口分开报告。
</p>

<p align="center">
  <strong>简体中文</strong> · <a href="README.en.md">English</a> ·
  <a href="https://github.com/2025chunxi/github-credibility-check/releases/tag/v0.1.0-beta">下载 v0.1.0-beta</a> ·
  <a href="https://github.com/2025chunxi/github-credibility-check/issues">反馈问题</a>
</p>
...
| 只看 Star 的工具 | `github-credibility-check` |
|---|---|
...
```text
使用 $github-credibility-check 以 standard 模式评估 https://github.com/owner/repo。
把可信度和采用风险分开，并明确证据来源、置信度和缺失信息。
```

### 方式二：下载发布包

下载 [`github-credibility-check.skill`](https://github.com/2025chunxi/github-credibility-check/releases/download/v0.1.0-beta/github-credibility-check.skill)，将归档中的 `github-credibility-check` 顶层目录解压到 `$CODEX_HOME/skills`（默认 `~/.codex/skills`），然后新建 Codex 任务以加载 Skill。
...
输出为 `dist/github-credibility-check.skill`。构建会执行评分回归测试、严格 Skill 校验、8 个校准案例、归档完整性检查，以及仓库级密钥、PII、本机路径和归档安全扫描。CI 不执行实时 GitHub 请求。
...
当前版本为 `v0.1.0-beta`。证据采集、确定性评分和校准流程已有自动化测试；启发式结论是可复核的决策支持，不是操纵行为或生产安全性的证明。

欢迎提交 [Issue](https://github.com/2025chunxi/github-credibility-check/issues) 或阅读 [CONTRIBUTING.md](CONTRIBUTING.md) 参与改进。
